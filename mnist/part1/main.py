...
#rint('Multiclass SVM test_error:', run_multiclass_svm_on_MNIST())
...
#print("test error for mod3 function:", run_softmax_on_MNIST_mod3())
...
#       and evaluate its accuracy on (test_pca, test_y).train_x, train_y, test_x, test_y = get_MNIST_data()
train_x, train_y, test_x, test_y = get_MNIST_data()
...
#print("test_error for TESTING PCA:",test_error)
...
classifier = LinearSVC(random_state=0,kernel='poly',degree=3)
classifier.fit(train_pca,train_y)
predictions=classifier.predict(test_y)
svm_error=compute_test_error_svm(test_y, predictions)
print("svm_error ", svm_error)